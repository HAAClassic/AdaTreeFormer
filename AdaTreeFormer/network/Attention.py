...
class QKV(nn.Module):
    ...
    def forward(self, query_img, support_img):
        ...
        savemat('DA_ST.mat', {'query':[np.squeeze(tensor.cpu().data.numpy()) for tensor in query_feats], 'support': [np.squeeze(tensor.cpu().data.numpy()) for tensor in support_feats]})
        savemat('qkv.mat', { 's2s_mask': np.squeeze(s2s_mask.cpu().data.numpy()), 't2t_mask': np.squeeze(t2t_mask.cpu().data.numpy()), 't2s_mask': np.squeeze(t2s_mask.cpu().data.numpy()), 's2t_mask': np.squeeze(s2t_mask.cpu().data.numpy())})
...
class QKV_model(nn.Module):
    ...
    def forward(self, query_feats, support_feats):
        ...
        #out = self.mixer2(self.mixer1(mix))
